...
#it prints specified header i.e. Server from the url mentioned

print(s.headers['Content-Type'])
response = s.json()
#json is header value of content type,and using requests library we can access header value
...
#we uploaded our dictionar as argument on site
...
#opposite of get, as post is used to send data to the specified url
print(r.text)
...
print(r.history)
#history contains list of all response object on site to perform redirect

print(r.status_code)
r = requests.post('https://www.github.com',allow_redirects=False)
#we cancelled redirecting
